...
class RouteService:
    ...
    def calculate_route(self, 
        ...
        try:
            ...
            if self.gmaps:
                ...
                if not directions:
                    # Metro cannot reach this destination - return None to exclude it
                    return None
                ...
                # Verify that the requested transit mode is available in the route
                if transport_mode == TransportMode.METRO and not self._has_metro_in_route(route_summary):
                    # Metro not available in this route - return None to exclude metro entirely
                    return None
            ...
            print(f"Error calculating route for {mode}: {e}")
            return None
    ...
    def _has_metro_in_route(self, route_summary: Dict[str, Any]) -> bool:
        """Check if metro (subway) is included in the route."""
        transit_segments = route_summary.get('transit_segments', [])
        if not transit_segments:
            return False
        
        for segment in transit_segments:
            vehicle_type = segment.get('vehicle', '').lower()
            line_name = segment.get('line', '').lower()
            
            # Check for metro/subway indicators
            if 'subway' in vehicle_type or 'metro' in vehicle_type:
                return True
            if 'metro' in line_name:
                return True
