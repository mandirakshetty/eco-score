...
class GoogleMapsClient:
    ...
    def get_directions(self, origin: Union[str, Tuple], destination: Union[str, Tuple], 
        ...
        try:
            ...
            transit_mode = None
            if mode == "transit":
                # Use provided transit_modes or default to all
                transit_mode = transit_modes if transit_modes else ['bus', 'subway', 'train', 'tram']
...
class RouteService:
    ...
    def calculate_route(self, start_coords: Tuple[float, float], 
        ...
        try:
            # Determine transit modes based on mode type
            transit_modes = None
            if mode == "metro":
                # Only request metro/subway
            ...
            # Verify the route actually contains the requested mode
            if not self._verify_route_mode(route_data, mode):
                # If requested mode not found in route, fallback to basic route
                return self._create_basic_route(start_coords, end_coords, mode)
    ...
    def _verify_route_mode(self, route_data: Dict, requested_mode: str) -> bool:
        """Verify that the route contains the requested transit mode."""
        if requested_mode in ["car", "walking", "bicycle"]:
            # Non-transit modes are always valid
            return True
        
        if requested_mode == "metro":
            # Check if route contains subway/metro
            leg = route_data.get('legs', [{}])[0]
            for step in leg.get('steps', []):
                if step['travel_mode'] == 'TRANSIT' and 'transit_details' in step:
                    vehicle_type = step['transit_details'].get('line', {}).get('vehicle', {}).get('type', '').lower()
                    if vehicle_type == 'subway':
                        return True
            return False
        
        elif requested_mode == "bus":
            # Check if route contains bus
            leg = route_data.get('legs', [{}])[0]
            for step in leg.get('steps', []):
                if step['travel_mode'] == 'TRANSIT' and 'transit_details' in step:
                    vehicle_type = step['transit_details'].get('line', {}).get('vehicle', {}).get('type', '').lower()
    ...
    def _process_google_route(self, route_data: Dict, mode: str) -> Dict[str, Any]:
        ...
        for i, step in enumerate(leg['steps']):
            ...
            # Extract transit details with improved metro/bus distinction
            if step['travel_mode'] == 'TRANSIT' and 'transit_details' in step:
                transit = step['transit_details']
                vehicle_type = transit.get('line', {}).get('vehicle', {}).get('type', '').lower()
                
                # Accurately determine the mode based on vehicle type
                if vehicle_type == 'subway':
                    display_mode = 'metro'
                    icon = '🚇'
                    ...
                    # Default based on requested mode
                    if mode == 'metro':
                        display_mode = 'metro'
                        icon = '🚇'
                    elif mode == 'bus':
                ...
                # Extract line information with fallbacks
                line_name = transit.get('line', {}).get('short_name', 
                          transit.get('line', {}).get('name', 'Unknown Line'))
                ...
                transit_info = {
                    ...
                    'icon': icon,
                    'departure': transit.get('departure_stop', {}).get('name', 'Unknown Station'),
                    'arrival': transit.get('arrival_stop', {}).get('name', 'Unknown Station'),
        ...
        return {
            ...
            'is_realistic': True,
            'actual_mode': self._determine_actual_mode(transit_segments, mode)
        }
    ...
    def _determine_actual_mode(self, transit_segments: List[Dict], requested_mode: str) -> str:
        """Determine the actual transit mode from transit segments."""
        if not transit_segments:
            return requested_mode
        
        # Check what type of transit is actually in the route
        vehicle_types = set()
        for segment in transit_segments:
            vehicle_types.add(segment.get('vehicle_type', '').lower())
        
        # Return the most common mode in the route
        if 'subway' in vehicle_types and 'bus' not in vehicle_types:
            return 'metro'
        elif 'bus' in vehicle_types and 'subway' not in vehicle_types:
            return 'bus'
        elif 'subway' in vehicle_types and 'bus' in vehicle_types:
            # Mixed mode - return requested
            return requested_mode
...
class MapRenderer:
    ...
    def display_map(self, map_obj: folium.Map, width: int = 800, height: int = 500, key: str = None):
...
def init_session_state():
    ...
    if 'unavailable_modes' not in st.session_state:
        st.session_state.unavailable_modes = []
    ...
    try:
        ...
        for mode_str in modes:
            ...
            if route:
                ...
                # Track unavailable modes
                if mode_str == 'metro':
                    unavailable_modes.append('metro')
        
        if not routes:
            st.error(" No routes could be calculated. Please try different locations or modes.")
            return
        
        # Determine best route based on priority (excluding unavailable modes)
        available_routes = {k: v for k, v in routes.items() if k not in unavailable_modes}
        
        if not available_routes:
            st.error(" No available routes could be calculated for this destination.")
            return
        
        if priority == RoutePriority.ECO_FRIENDLY:
            best_mode = max(available_routes.items(), key=lambda x: x[1]['eco_score'])[0]
        elif priority == RoutePriority.FASTEST:
            best_mode = min(available_routes.items(), key=lambda x: x[1]['total_duration_min'])[0]
        elif priority == RoutePriority.CHEAPEST:
            best_mode = min(available_routes.items(), key=lambda x: x[1]['cost_inr'])[0]
        else:  # BALANCED
            ...
            best_mode = max(available_routes.items(), key=lambda x: balanced_score(x[1]))[0]
        
        # Store in session state
        st.session_state.routes = available_routes  # Store only available routes
        ...
        st.session_state.unavailable_modes = unavailable_modes
        
        # Show success message
        best_route = available_routes[best_mode]
        mode_config = Settings.TRANSPORT_MODES.get(best_mode, {})
        actual_mode = best_route.get('actual_mode', best_mode)
        actual_mode_config = Settings.TRANSPORT_MODES.get(actual_mode, mode_config)
        
        # Build detailed success message
        route_summary = f"\n**Recommended: {actual_mode_config.get('icon', '')} {actual_mode_config.get('name', best_mode)}**"
        if actual_mode != best_mode:
            route_summary += f" (Requested: {mode_config.get('icon', '')} {mode_config.get('name', best_mode)})"
        ...
        st.success(f" Found {len(available_routes)} available route(s)! {route_summary}")
        
        # Show warning if metro is unavailable
        if unavailable_modes and 'metro' in unavailable_modes:
            st.error("❌ **Metro Service Not Available**")
            st.warning(
                "The metro train service does not go to this destination. "
                "Metro is only available in specific coverage areas of the city. "
                "Showing best alternative eco-friendly routes below."
            )
...
def render_welcome_screen():
    ...
    map_renderer.display_map(m, width=800, height=400, key="welcome_map")
...
def render_route_display(has_api_key: bool, api_key: str):
    ...
    # Determine the actual detected mode (metro vs bus)
    actual_mode = route.get('actual_mode', route['mode'])
    actual_mode_config = Settings.TRANSPORT_MODES.get(actual_mode, mode_config)
    
    # Create a container for the entire route display
    with st.container():
        # Route Header - Show both requested and actual mode if different
        if actual_mode != route['mode']:
            st.markdown(f"""
            <div class="section-header">
                <h2>{actual_mode_config.get('icon', '')} {actual_mode_config.get('name', actual_mode.capitalize())} Route</h2>
                <p style="color: #666; margin: 0;">
                    {route['start_address']} → {route['end_address']}<br>
                    <em style="font-size: 0.9rem;">
                    🔍 Detected Mode: <strong>{actual_mode_config.get('name', actual_mode)}</strong>
        ...
        with tab1:
            ...
            if has_api_key and route.get('is_realistic', False):
                ...
                map_renderer.display_map(route_map, width=800, height=500, key="realistic_map")
                ...
                map_renderer.display_map(simple_map, width=800, height=500, key="simple_fallback_map")
        ...
        with tab2:
            ...
            # Simple view
            simple_map = map_renderer.create_route_map(route, show_transit=True)
            map_renderer.display_map(simple_map, width=800, height=500, key="simple_view_map")
        ...
        with tab3:
            ...
            st_folium(sat_map, width=800, height=500, key="satellite_map")
...
def render_route_breakdown(route: Dict[str, Any]):
    ...
    for step_info in breakdown:
        ...
        # Show transit details if available - IMPROVED: Better metro/bus distinction
        if "transit" in step_info:
            ...
            # Determine icon, color and CSS class based on actual vehicle type
            if display_mode == 'metro' or vehicle_type == 'subway':
                detail_class = "transit-detail-box metro-detail"
                detail_icon = "🚇"
                transport_type = "Metro (Subway)"
                color_code = '#800080'
            elif display_mode == 'bus' or vehicle_type == 'bus':
                detail_class = "transit-detail-box bus-detail"
                detail_icon = "🚌"
                transport_type = "Bus"
                color_code = '#1565C0'
            elif display_mode == 'train' or vehicle_type in ['train', 'rail']:
                detail_class = "transit-detail-box"
                detail_icon = "🚂"
                transport_type = "Train"
                color_code = '#008000'
            elif display_mode == 'tram' or vehicle_type == 'tram':
                detail_class = "transit-detail-box"
                detail_icon = "🚊"
                transport_type = "Tram"
                color_code = '#DC143C'
            ...
            st.markdown(f"""
            <div class="{detail_class}" style="border-left-color: {color_code}; background-color: rgba({int(color_code[1:3], 16)}, {int(color_code[3:5], 16)}, {int(color_code[5:7], 16)}, 0.1);">
...
def render_alternative_routes():
    ...
    # Since unavailable modes are already filtered from st.session_state.routes,
    # we don't need to check and exclude them again
    
    if len(st.session_state.routes) <= 1:
        st.info("✓ Only one mode available for this route.")
        return
    
    # Create comparison DataFrame with available routes only
    comparison_data = []
    ...
    # Show unavailable modes notice
    if st.session_state.unavailable_modes:
        st.markdown("---")
        st.info(
            f"ℹ️ **Note:** Metro service is not available to this destination. "
            f"Please choose from the available transport modes above."
        )
...
def render_transit_details(route: Dict[str, Any]):
    """Render transit-specific details with accurate metro/bus distinction."""
    route_service = RouteService()
    ...
    if transit_summary.get("transit_segments"):
        ...
        for i, segment in enumerate(transit_summary["transit_segments"], 1):
            ...
            # Determine icon, color and transport type name based on actual vehicle type
            if display_mode == 'metro' or vehicle_type == 'subway':
                vehicle_icon = '🚇'
                transport_type = 'Metro (Subway)'
                color = '#800080'
            ...
            <div style="padding: 12px; margin: 8px 0; border-left: 5px solid {color}; 
                        ...
                        border-radius: 6px;">
                <b>Segment {i}: {vehicle_icon} {transport_type} - Line {segment.get('line', 'Unknown')}</b><br>
                <b>Vehicle Type:</b> {vehicle_type.title() if vehicle_type else 'Unknown'}<br>
                <b>From:</b> {segment.get('departure', 'Unknown')}{departure_time}<br>
                <b>To:</b> {segment.get('arrival', 'Unknown')}{arrival_time}<br>
                <b>Stops:</b> {segment.get('stops', 0)} | <b>Direction:</b> {segment.get('headsign', 'Unknown')}
            </div>
            """, unsafe_allow_html=True)
